...
Remotely **mute or bypass any node in your workflow by its node ID** — even nodes buried inside nested Subgraphs — from a single compact control node. The node toggles and target pickers are standard ComfyUI widgets, so they can be **linked or promoted to Subgraph nodes** and used as top-level switches.
